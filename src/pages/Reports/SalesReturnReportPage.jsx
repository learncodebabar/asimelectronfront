...
export default function SalesReturnReportPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      // Use the new SALE_RETURNS endpoint
      const [returnsRes, customersRes] = await Promise.all([
        api.get(EP.SALE_RETURNS.GET_ALL),
        api.get(EP.CUSTOMERS.GET_ALL)
      ]);
      
      if (returnsRes.data.success && returnsRes.data.data) {
        setReturns(returnsRes.data.data);
        console.log("Found sale returns:", returnsRes.data.data.length);
      } else {
        setReturns([]);
      }
      ...
      // Fallback: try to get from sales with return type
      try {
        const fallbackRes = await api.get(EP.SALES.GET_ALL);
        if (fallbackRes.data.success && fallbackRes.data.data) {
          const saleReturns = fallbackRes.data.data.filter(s => 
            s.saleType === "return" || s.type === "return"
          );
          setReturns(saleReturns);
          console.log("Found returns from fallback:", saleReturns.length);
        }
      } catch (fallbackError) {
  ...
  const filteredReturns = returns.filter(r => {
    ...
    const matchesSearch = 
      ...
      (r.saleInvNo || r.originalInvoiceNo || "").toLowerCase().includes(search.toLowerCase());
  ...
  const totalAmount = filteredReturns.reduce((sum, r) => sum + (r.netTotal || r.subTotal || 0), 0);
  ...
  const buildPrintHtml = () => {
    const rows = filteredReturns.map((r, i) => `
      <tr>
        ...
        <td style="padding:10px;border:2px solid #000;font-size:13px">${(r.returnDate || r.invoiceDate)?.split("T")[0] || r.returnDate}</td>
        <td style="padding:10px;border:2px solid #000;font-size:13px;font-weight:bold">${r.customerName || "—"}</td>
        <td style="padding:10px;border:2px solid #000;font-size:13px;font-family:monospace">${r.saleInvNo || r.originalInvoiceNo || "—"}</td>
        <td style="padding:10px;border:2px solid #000;text-align:center;font-size:13px">${r.items?.length || 0}<td>
        <td style="padding:10px;border:2px solid #000;text-align:right;font-size:13px;font-weight:bold;color:#dc2626">${fmt(r.netTotal || r.subTotal || 0)}</td>
      </tr>
    `).join("");
    ...
    <body>
      ...
      <div class="footer">Printed on: ${new Date().toLocaleString()} | ${SHOP_INFO.devBy}</div>
    </body>
    </html>`;
  };
  ...
  const buildSingleReturnHtml = (returnItem) => {
    ...
    <head>
      ...
      <style>
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .info{display:flex;justify-content:space-between;margin:15px 0;padding:10px;background:#f8fafc;border:1px solid #000;flex-wrap:wrap;gap:10px}
        table{width:100%;border-collapse:collapse;margin:15px 0}
    ...
    <body>
      ...
      <div class="info">
        ...
        <div><strong>Date:</strong> ${(returnItem.returnDate || returnItem.invoiceDate)?.split("T")[0] || returnItem.returnDate}</div>
        <div><strong>Customer:</strong> ${returnItem.customerName || "—"}</div>
        <div><strong>Original Invoice:</strong> ${returnItem.saleInvNo || returnItem.originalInvoiceNo || "—"}</div>
      </div>
      <table>
        <thead><tr><th>#</th><th>CODE</th><th>PRODUCT</th><th>UOM</th><th>QTY</th><th>RATE</th><th>AMOUNT</th></tr></thead>
        <tbody>${rows}</tbody>
      </table>
      <div class="total">TOTAL RETURN AMOUNT: PKR ${fmt(returnItem.netTotal || returnItem.subTotal || 0)}</div>
      <div class="footer">Printed on: ${new Date().toLocaleString()} | ${SHOP_INFO.devBy}</div>
    </body>
    </html>`;
  };
    ...
    <div className="sl-page">
      <div className="xp-titlebar" style={{ background: "#dc2626", padding: "12px 20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          ...
          <span className="xp-tb-title" style={{ color: "white", fontSize: "16px", fontWeight: "bold" }}>Sales Return Report — DGS Store</span>
        </div>
        <div className="xp-tb-actions">
          <button className="xp-btn xp-btn-sm" onClick={fetchData} style={{ fontSize: "12px", padding: "6px 12px", fontWeight: "bold", background: "rgba(255,255,255,0.2)", border: "none", color: "white", cursor: "pointer" }}>⟳ Refresh</button>
          <button className="xp-btn xp-btn-sm" onClick={handlePrint} disabled={!filteredReturns.length} style={{ fontSize: "12px", padding: "6px 12px", fontWeight: "bold", background: "rgba(255,255,255,0.2)", border: "none", color: "white", cursor: "pointer", marginLeft: "8px" }}>🖨 Print</button>
        </div>
      </div>
      ...
      <div style={{ padding: "16px" }}>
        ...
        <div style={{ background: "#f8fafc", borderRadius: "8px", padding: "16px", marginBottom: "20px", border: "2px solid #dc2626" }}>
          <div style={{ display: "flex", gap: "12px", alignItems: "flex-end", flexWrap: "wrap" }}>
            ...
            <div>
              <button className="xp-btn xp-btn-sm" onClick={clearFilters} style={{ padding: "8px 16px", fontWeight: "bold", border: "1px solid #dc2626", borderRadius: "4px", cursor: "pointer" }}>Clear Filters</button>
            </div>
          </div>
        </div>
        ...
        {/* Results Table */}
        <div className="xp-table-panel" style={{ border: "2px solid #dc2626", borderRadius: "8px", overflow: "hidden" }}>
              ...
              <tbody>
                ...
                {filteredReturns.map((r, i) => (
                  <tr key={r._id} style={{ borderBottom: "1px solid #e2e8f0" }}>
                    ...
                    <td style={{ padding: "8px", border: "1px solid #dc2626" }}>{(r.returnDate || r.invoiceDate)?.split("T")[0] || r.returnDate}</td>
                    <td style={{ padding: "8px", border: "1px solid #dc2626", fontWeight: "bold" }}>{r.customerName || "—"}</td>
                    <td style={{ padding: "8px", border: "1px solid #dc2626", fontFamily: "monospace" }}>{r.saleInvNo || r.originalInvoiceNo || "—"}</td>
                    <td style={{ padding: "8px", border: "1px solid #dc2626", textAlign: "center" }}>{r.items?.length || 0}</td>
                    <td style={{ padding: "8px", border: "1px solid #dc2626", textAlign: "right", fontWeight: "bold", color: "#dc2626" }}>{fmt(r.netTotal || r.subTotal || 0)}</td>
                    <td style={{ padding: "8px", border: "1px solid #dc2626", textAlign: "center" }}>
                      <button className="xp-btn xp-btn-sm" style={{ fontSize: "10px", padding: "4px 8px", background: "#dc2626", color: "white", border: "none", borderRadius: "4px", cursor: "pointer" }} onClick={() => handleViewReturn(r)}>VIEW</button>
                    </td>
                  </tr>
                ))}
              </tbody>
