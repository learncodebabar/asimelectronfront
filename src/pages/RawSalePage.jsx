...
const HOLD_KEY = "asim_hold_bills_rawsale_v1";
  ...
  if (type === "Thermal") {
    ...
    return `<!DOCTYPE html><html><head><meta charset="utf-8">${GOOGLE_FONT_LINK}<style>
      *{box-sizing:border-box;margin:0;padding:0}
      ...
      <div class="meta-row">
        <span><b>Raw Sale Invoice</b></span>
        <span>ADMIN</span>
...
/* ══════════════════════════════════════════════════════════
   SAVE CONFIRM MODAL - Only for Cash Customers
══════════════════════════════════════════════════════════ */
    ...
    <div className="scm-overlay">
      <div className="scm-window">
        <div className="scm-tb">
          ...
          <span className="scm-tb-title">Raw Sale Invoice— {salePayload.invoiceNo} &nbsp;|&nbsp; {salePayload.customerName}</span>
          <button className="xp-cap-btn xp-cap-close" onClick={onClose}>✕</button>
        </div>
        ...
        <div className="scm-amounts">
          ...
          <div className="scm-box" style={{ borderLeft: "none" }}><div className="scm-box-label">Received</div><input ref={paidRef} type="text" className="scm-recv-input" value={paidAmount} onChange={(e) => setPaidAmount(e.target.value)} onFocus={(e) => e.target.select()} /></div>
        ...
        <div className="scm-hint">↵ Enter (in Received field) = Save &amp; Print &nbsp;|&nbsp; Esc = Return to Invoice</div>
      </div>
    </div>
  );
}
...
/* ══════════════════════════════════════════════════════════
   PRODUCT SEARCH MODAL - Only shows products with company name "raw"
══════════════════════════════════════════════════════════ */
...
/* ══════════════════════════════════════════════════════════
   CUSTOMER DROPDOWN - Shows ALL suppliers (raw-sale, supplier, credit)
══════════════════════════════════════════════════════════ */
    ...
    <>
      <div className={`sl-page${creditWarning ? " sl-credit-mode" : ""}`}>
        ...
        <div className="sl-body">
          <div className="sl-left">
            ...
            <div className="sl-summary-bar">
              ...
              <div className="sl-sum-cell"><label>Extra Discount</label><input ref={discRef} type="text" className="sl-sum-input" style={{ background: "#fffde7" }} value={extraDiscount} min={0} onChange={(e) => setExtraDiscount(e.target.value)} onKeyDown={(e) => e.key === "Enter" && receivedRef.current?.focus()} onFocus={(e) => e.target.select()} /></div>
              <div className="sl-sum-cell"><label>Received</label><input ref={receivedRef} type="text" className="sl-sum-input" style={{ color: "var(--xp-green)", fontWeight: 700, background: "#fffde7" }} value={received} min={0} onChange={(e) => setReceived(e.target.value)} onKeyDown={(e) => e.key === "Enter" && saveRef.current?.focus()} onFocus={(e) => e.target.select()} /></div>
            ...
            <div className="sl-customer-bar">
              <div className="sl-cust-cell"><label>Code</label><input className="sl-cust-input" style={{ width: 65, background: "#fffde7" }} value={codeSearch} onChange={(e) => setCodeSearch(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); const q = codeSearch.trim(); if (!q) return; const found = allCustomers.find((c) => String(c.code).toLowerCase() === q.toLowerCase() && (c.customerType || c.type || "").toLowerCase() === "raw-sale"); if (found) { handleCustomerSelect(found); setCodeSearch(""); } else { showMsg(`Code "${q}" — customer nahi mila`, "error"); } } }} autoComplete="off" /></div>
              <div className="sl-cust-cell sl-cust-buyer"><label>Customer Name</label><CustomerDropdown allCustomers={allCustomers} value={customerId} displayName={buyerName} customerType={customerType} onSelect={handleCustomerSelect} onClear={handleCustomerClear} onAddNew={handleAddNewCustomer} allowedTypes={["raw-sale", "supplier", "credit"]} /></div>
              <div className="sl-cust-cell"><label>Prev Balance</label><input type="text" className="sl-cust-input" style={{ width: 85, background: "#fffde7" }} value={prevBalance} onChange={(e) => setPrevBalance(e.target.value)} onFocus={(e) => e.target.select()} /></div>
              <div className="sl-cust-cell"><label>Net Recv.</label><input className="sl-cust-input sl-net-recv" style={{ color: balance > 0 ? "var(--xp-red)" : "var(--xp-green)", fontWeight: 700, width: 85 }} value={Number(balance).toLocaleString("en-PK")} readOnly /></div>
              <div className="sl-pay-btns">{["Cash", "Credit", "Bank", "Cheque"].map((m) => (<button key={m} className={`sl-pay-btn${paymentMode === m ? " active-" + m.toLowerCase() : ""}`} onClick={() => handlePaymentMode(m)}>{m}</button>))}</div>
            </div>
        ...
        <div className="sl-cmd-bar">
          ...
          <button className="xp-btn xp-btn-danger xp-btn-sm" disabled={!editId} onClick={async () => { if (!editId || !window.confirm("Delete this sale?")) return; try { await api.delete(EP.RAW_SALES.DELETE(editId)); showMsg("Sale deleted"); fullReset(); refreshInvoiceNo(); } catch { showMsg("Delete failed", "error"); } }}>Delete Record</button>
          <div className="xp-toolbar-divider" />
          <div className="sl-cmd-checks"><label className="sl-check-label"><input type="checkbox" checked={sendSms} onChange={(e) => setSendSms(e.target.checked)} /> Send SMS</label><label className="sl-check-label"><input type="checkbox" /> Print P.Bal</label></div>
          <div className="xp-toolbar-divider" />
          ...
          <div className="xp-toolbar-divider" />
          <span className={`sl-inv-info${editId ? " edit-mode" : ""}`}>{editId ? "✏ Editing sale record" : `${invoiceNo} | Items: ${items.length} | Total: ${Number(subTotal).toLocaleString("en-PK")} | ${saleSource} / ${paymentMode}`}</span>
      ...
      <style>{`
        ...
        input, .xp-input, .sl-product-input, .sl-num-input, .sl-sum-input, .sl-cust-input, .sl-credit-statement-input, .sl-inv-input-large, .sl-date-input, .sl-sum-val { border-color: #000000 !important; border-width: 1px !important; border-style: solid !important; background: #ffffff !important; }
        ...
        .sl-inv-input-large { font-size: 16px !important; font-weight: bold !important; width: 180px !important; text-align: center !important; background: #ffffff !important; padding-left: 32px !important; padding-right: 32px !important; }
